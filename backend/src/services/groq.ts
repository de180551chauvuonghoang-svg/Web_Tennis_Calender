...
2. studentPhone: Số điện thoại của học viên (nếu có trong tin nhắn). GHI LẠI CHỈNH XÁC TẤT CẢ CÁC CHỮ SỐ của số điện thoại, bao gồm cả mã quốc gia. Ví dụ: "84 785669776" → "84785669776", "86 138 2430 1352" → "8613824301352", "+852 6041 6779" → "85260416779", "0795552428" → "0795552428". Loại bỏ mọi dấu cách, gạch nối, ngoặc, dấu cộng. Nếu không có số điện thoại nào trong tin nhắn, để là chuỗi rỗng "".
3. totalSessions: Tổng số buổi học mà học viên đăng kí (số nguyên, ví dụ "3 buổi" = 3, "5 buổi" = 5). Nếu không đề cập, để là 0.
4. currentSession: Số thứ tự buổi tập hiện tại của học viên (ví dụ: "buổi thứ 5" = 5, "buổi 5" = 5, "buổi thứ 1" = 1). Nếu không đề cập, để là 0.
5. startTime: Thời gian bắt đầu buổi tập (Định dạng chuỗi ISO địa phương Việt Nam: YYYY-MM-DDTHH:mm:ss, tuyệt đối KHÔNG có chữ 'Z' hay múi giờ lệch ở cuối). Bạn cần phân tích kỹ mốc thời gian HLV ghi (ví dụ: "6 AM", "3h30pm", "4:00 chiều", "sáng mai", "tối thứ 5"). Kết hợp với ngày hiện tại (${currentDateStr}) để đưa ra giờ địa phương chính xác nhất ở Việt Nam (Ví dụ: "6 AM ngày 17/7" -> "2026-07-17T06:00:00").
6. duration: Thời lượng buổi tập tính bằng phút (dạng số, ví dụ "1h" = 60, "1h30p" = 90, "2h" = 120. Mặc định là 60 nếu không nhắc đến hoặc không đề cập thời lượng).
...
Yêu cầu định dạng trả về dưới dạng JSON:
{
 ...
 Chỉ trả về JSON, không kèm giải thích hay Markdown tags nào khác bên ngoài JSON.
    ...
    let startTimeVal = parsed.startTime || new Date().toISOString();
